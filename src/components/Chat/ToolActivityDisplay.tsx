 ...
 * Supports enhanced streaming with thinking content and tool argument previews
 */
...
import {
  ThinkingIndicator,
  ThinkingIndicatorCompact,
} from "./ThinkingIndicator";
import { ToolPreview, ToolPreviewCompact } from "./ToolPreview";
...
interface ToolActivityDisplayProps {
  ...
  /** Thinking/reasoning content being streamed (for enhanced streaming) */
  thinkingContent?: string;
  /** Full thinking data from the current message */
  thinkingData?: ThinkingData;
  /** Status message from the server */
  statusMessage?: string | null;
  className?: string;
  /** Display mode: 'full' shows detailed previews, 'compact' shows minimal UI */
...
export function ToolActivityDisplay({
  ...
  // Check if we have any tools with partial args (streaming)
  const streamingTools = activeToolCalls.filter(
    (t) => t.partialArgs && t.status !== "success" && t.status !== "error",
  );
  ...
  if (displayMode === "compact") {
    return (
      ...
      >
        ...
        {(thinkingData?.isActive || thinkingContent) && (
          <ThinkingIndicatorCompact isActive={true} />
        )}

        {/* Status message */}
        <AnimatePresence mode="wait">
          {statusMessage && !thinkingData?.isActive && (
            <motion.div
              key={statusMessage}
              ...
              className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-slate-100 text-slate-600 text-xs"
            >
        ...
        {/* Tool previews compact */}
        {activeToolCalls.map((tool) => (
          <ToolPreviewCompact key={tool.id} toolCall={tool} />
        ))}
      </motion.div>
    );
  }

  // Full mode with detailed previews
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -10 }}
      className={cn(
        "rounded-xl border bg-gradient-to-br from-slate-50 to-blue-50 border-slate-200 shadow-sm space-y-3",
        className,
      )}
    >
      {/* Thinking indicator with content */}
      {(thinkingData?.isActive || thinkingData?.content || thinkingContent) && (
        <ThinkingIndicator
          thinking={
            thinkingData || { isActive: true, content: thinkingContent || "" }
          }
          className="m-3"
        />
      )}

      {/* Status/thinking phase indicator (when no full thinking content) */}
      {thinkingPhase && !thinkingData?.isActive && !thinkingContent && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      <AnimatePresence mode="wait">
        {statusMessage && !thinkingPhase && !thinkingData?.isActive && (
          <motion.div
            key={statusMessage}
      ...
      {/* Streaming tool previews (showing args being generated) */}
      {streamingTools.length > 0 && (
        <div className="px-3 space-y-2">
          {streamingTools.map((tool) => (
            <ToolPreview key={tool.id} toolCall={tool} />
          ))}
        </div>
      )}

      {/* Active tool calls (non-streaming) */}
      {activeToolCalls.length > 0 && streamingTools.length === 0 && (
