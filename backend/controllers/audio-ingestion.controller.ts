...
import {
  ...
  DeviceCapabilities,
  audioSessionManager,
} from "../services/audio-session-manager.js";
import { ConnectionProtocol, DeviceType } from "@prisma/client";
 ...
 * Supports both Authorization header and query parameter (for SSE/polling)
 */
const requireAuth = (
  ...
  // Try to extract token from query parameter (for SSE/polling requests)
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { deviceId, deviceToken, audioFormat, preferredProtocol } =
        req.body;
      ...
      if (!resolvedDeviceId) {
        return res
          .status(400)
          .json({ error: "deviceId or deviceToken required" });
      }
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!result.success) {
        return res
          .status(404)
          .json({ error: "Session not found or cannot be resumed" });
      }
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const isFinal =
        req.headers["x-is-final"] === "true" || req.body?.isFinal === true;
      const audioFormat =
 ...
 */
router.get(
  "/sessions/:sessionId/events/poll",
  requireAuth,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const sessionId = req.params.sessionId;
      ...
      const events = audioSessionManager.getEventsForPolling(
        sessionId,
        afterEventId,
      );
      ...
      res.json({
        events,
        lastEventId:
          events.length > 0 ? events[events.length - 1].id : afterEventId || 0,
