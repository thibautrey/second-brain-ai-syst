...
export class ToolExecutorService {
  ...
  private async executeBuiltinTool(
    ...
    switch (toolId) {
      ...
      case "skills_management":
        return this.executeSkillsManagementAction(userId, action, params);
      case "read_tool_code":
  ...
  private async executeGeneratedTool(
    ...
    // Get the tool to access its schema for validation
    ...
    if (!skipValidation && tool.inputSchema) {
      ...
      if (isValidGeneratedToolSchema(schema)) {
        ...
        if (!validationResult.success) {
          // Return validation error formatted for LLM retry
          const errorMessage = [
  ...
  /**
   * Execute skills_management actions
   * Create, list, update, delete, install, and uninstall skills
   */
  private async executeSkillsManagementAction(
    userId: string,
    action: string,
    params: Record<string, any>,
  ): Promise<any> {
    // Import skill manager dynamically to avoid circular dependency
    const { skillManager, SkillCategory } = await import("./skill-manager.js");
    ...
    switch (action) {
      ...
      case "get": {
        ...
        // Get full content for installed skill
    ...
    return [
      ...
      },
      {
        ...
        description:
          ...
          "Use for: browsing the web, navigating dynamic sites, extracting content from JavaScript-rendered pages, taking screenshots, " +
      ...
      },
      {
        name: "read_skill",
        description:
          "Read skill instructions for specialized workflows. Use when a skill from the available_skills list applies to the user's request. " +
          "Read the skill's instructions BEFORE proceeding with the task to ensure you follow the correct workflow. " +
          "Skills contain step-by-step guides for common tasks.",
        parameters: {
          type: "object",
          properties: {
            action: {
      ...
      },
      {
        ...
        description:
          "Manage skills - list, install, create, update, and delete skills. Skills are reusable instruction sets that guide how to accomplish specific tasks. " +
          "Use 'list_installed' to see what skills the user has. Use 'create' to make new custom skills for recurring workflows. " +
          "Custom skills can be updated/deleted; hub skills can only be installed/uninstalled.",
        parameters: {
          type: "object",
          properties: {
            action: {
              ...
              description:
                "'list_installed': show user's installed skills. " +
                "'list_hub': browse skills in the hub. " +
                "'list_custom': show only user-created custom skills. " +
                "'get': get full details of a skill including instructions. " +
                "'install': install a skill from the hub. " +
                "'uninstall': remove an installed skill. " +
                "'toggle': enable/disable a skill. " +
                "'create': create a new custom skill (requires slug, name, description, instructions). " +
                "'update': modify a custom skill. " +
                "'delete': permanently remove a custom skill.",
            },
            slug: {
              type: "string",
              description:
                "Skill identifier. For 'create': must be lowercase with only letters, numbers, dashes. " +
                "For other actions: the slug of the target skill.",
            },
            name: {
              type: "string",
              description:
            ...
            description: {
              type: "string",
              description:
                "Brief description of what the skill does (for 'create' and 'update'). " +
                "Example: 'Help prepare and run daily standup meetings with automated notes'",
            },
            instructions: {
              type: "string",
              description:
                "Markdown instructions for the skill (for 'create' and 'update'). " +
                "This is what the AI reads when the skill is activated. Include step-by-step workflows, " +
                "which tools to use, and any important guidelines. Can include headers, lists, code blocks.",
            },
            icon: {
              type: "string",
              description:
            ...
            config: {
              type: "object",
              description:
                "For 'install': optional configuration to pass to the skill",
            },
          },
          required: ["action"],
        },
